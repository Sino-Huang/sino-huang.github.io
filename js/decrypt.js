...
const _click_handler = function (element) {
    ...
    // add event 
    if (typeof decryptedcustomevent !== "undefined"){
        document.dispatchEvent(decryptedcustomevent);
        console.log("dispatchEvent: hugo-text-decrypted-alert at button");
    }
...
window.onload = () => {
    ...
    if (index > 0){
        if (typeof decryptedcustomevent !== "undefined"){
            document.dispatchEvent(decryptedcustomevent);
            console.log("dispatchEvent: hugo-text-decrypted-alert has pwd");
        }
    }
};